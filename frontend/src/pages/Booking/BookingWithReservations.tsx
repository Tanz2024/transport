...
const Booking: React.FC = () => {
  ...
  const getPrice = (row: number, className: Seat["className"]) => {
    if (vehicleType === "bus") {
  ...
  // Production-level booking payload builder
  const buildBookingPayload = (scheduleId: number, seatNumbers: number[]) => ({
  ...
  const confirm = () => {
    ...
    if (type === "return" || type === "round") {
      submitBooking(true, selected);
    } else {
      setShowRoundTripPrompt(true);
    }
  };

  const submitBooking = async (isReturn: boolean, seatsToBook: Seat[]) => {
    const token = sessionStorage.getItem("token");

    try {
      // Production booking submission
      if (isReturn) {
        // Round-trip booking
        const raw = JSON.parse(sessionStorage.getItem("outboundTrip") || "{}");
        const outboundNums: number[] = raw.seat_numbers;
        
        if (!raw.schedule_id || !Array.isArray(outboundNums) || !outboundNums.length) {
        ...
        const outboundPayload = {
          ...
          seat_numbers: outboundNums
        };
        const returnPayload = {
          schedule_id: Number(id),
          seat_numbers: seatsToBook.map(s => s.seatNumber)
        };

        const res = await fetch("http://localhost:5000/api/round-trip-bookings", {
          method: "POST",
          headers: {
        ...
        if (!res.ok) {
          ...
          throw new Error(err.error || "Round-trip booking failed");
        }
        ...
        const payload = buildBookingPayload(Number(id!), seatsToBook.map(s => s.seatNumber));

        const oneWayRes = await fetch("http://localhost:5000/api/bookings", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            ...(token && { Authorization: `Bearer ${token}` }),
        ...
        if (oneWayRes.status === 409) {
          const err = await oneWayRes.json();
          return alert(err.error || "One or more seats just sold out.");
        }
        if (oneWayRes.status === 400) {
          const err = await oneWayRes.json();
          return alert(err.error || "Invalid booking data.");
        }
        if (!oneWayRes.ok) {
          throw new Error("Booking failed. Please try again.");
        }

        // Navigate to confirmation with one-way trip
  ...
  return (
    <div style={{ padding: 20, maxWidth: 960, margin: "auto" }}>
      ...
      {type !== "return" && type !== "round" && showRoundTripPrompt && (
        <div
          style={{
            position: "fixed",
            top: 0,
        ...
        >
          ...
          >
            ...
            >
              ...
              <button
                onClick={() => {
                  submitBooking(false, bookingDetails);
                }}
                style={{
